...
async function handleProxy(request: Request, { params }: { params: { path: string[] } }) {
  ...
  // Translate frontend proxy path to correct Laravel V1 versioned backend path
  ...
  try {
    ...
    // 2. Fallback to mock if API returns 404 (endpoint not implemented) or 401 (unauthorized mock session)
